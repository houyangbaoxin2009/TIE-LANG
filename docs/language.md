...
> 表的运行时生成（存取、遍历）属 M2 里程碑，当前 IR 阶段对表字面量报"未实现"。
...
| `import`   | 导入其他 tie 文件（M2 规划）          | `import "./x.tie" as x`     |
| `as`       | 导入别名（M2 规划）                 | `import "./x.tie" as x`     |
