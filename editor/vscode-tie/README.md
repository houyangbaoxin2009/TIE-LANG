...
| 功能 | 说明 |
| --- | --- |
| 语法高亮 | 关键词 / 类型 / 运算符 / 数字（含 0x 与下划线分隔）/ 字符串 / 注释 / 头指令（`// tie:xxx` 特殊着色）/ 内置函数 |
| 智能缩进 | 函数 / 类 / 控制流行尾 `{` 自动缩进，`}` 自动对齐，`case` / `default` 自动缩进 |
| 代码片段 | `func`、`class`、`if` / `ifelse`、`for`（范围遍历）、`while`、`switch`、`method`、`static method`、`import`、`println` |
